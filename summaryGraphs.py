...
class SummaryGraph(object):
    ...
    def histo(self,type1,type2,links):
        ...
        v = self.vertices[type2].items()
        if 'shape' in self.vprop[type2]:
            i = self.vprop[type2].index('shape')-1
            for n in range(len(v)):
                v[n] = list(v[n])
                v[n][1][i]='triangle'
                v[n][0]='*'+v[n][0][0]
        else:
            v = [x[1].append('triangle') for x in v]
            histograph.vprop[type2].append('shape')
            for n in range(len(v)):
                v[n] = list(v[n])
                v[n][0]='*'+v[n][0][0]
        histograph.vertices[type2] = dict(v)    
...
if __name__ == '__main__':
    ...
    print '\nTest the generation of the pads for the 3 types of graphs\n'
    histograph = h.histo('Personne','Région'.decode('utf-8'),'PersonRegion')
