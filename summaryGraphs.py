...
class SummaryGraph(object):
    ...
    def norm_histo(self,type1,type2,prop,links):
        ...
        for x in self.edges[links]:    
            ...
            #counts[x[0][order[1]]]= counts[x[0][order[1]]]/self.
        for x in counts:
            counts[x]=[counts[x],round(counts[x]/,2)]
        ...
        for x in pnodes:
            if len(pnodes[x])>0:
                idnode = str(x[0])+'_to_'+str(x[1])
                sci.append((x[0],idnode))
                nhist.vertices['Percent']['*'+idnode]=[str(len(pnodes[x])),'http://icons.iconarchive.com/icons/iconarchive/red-orb-alphabet/256/Percent-icon.png', 'circle']
